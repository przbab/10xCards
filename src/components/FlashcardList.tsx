        ...
        <div data-test-id="flashcard-list">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {cards.map((card) => (
                    <Card
                        actions={[
                            { 'data-test-id': 'edit-button', id: 'edit', text: 'Edit', variant: 'default' },
                            { 'data-test-id': 'delete-button', id: 'delete', text: 'Delete', variant: 'destructive' },
                        ]}
                        back={card.back || 'No back text available'}
                        data-test-id={`flashcard-${card.id}`}
                        front={card.front}
                        id={card.id}
                        key={card.id}
